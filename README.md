...
* [`enums` for your CoreData model](#coredata).
* [machine and human code for your CoreData model](#coredata-machine-and-human-code).
...
```sh
swiftgen coredata model /path/to/coredata/model
```
This command defines for every entity struct with CoreData entity name and nested struct with all relationships/attributes/fetched properties names.
...
```sh
swiftgen coredata human --output "./Human" --file-mask "{{class}}+CoreDataClass.swift" /path/to/coredata/model
swiftgen coredata machine --output "./Machine" --file-mask "{{class}}+CoreDataProperties.swift" /path/to/coredata/model
```

These commands generate for every entity swift NSManagedObject subclass. Human files are used for user editing. Machine are regenerated every time, and should not be changed.
...
It also relies on [`Stencil`](https://github.com/kylef/Stencil/blob/master/LICENSE), [`SWXMLHash`](https://github.com/drmohundro/SWXMLHash/blob/master/LICENSE), [`Commander`](https://github.com/kylef/Commander/blob/master/LICENSE) and [`PathKit`](https://github.com/kylef/PathKit/blob/master/LICENSE) licenses.
