...
Currently, if you run the ModelTests, it will print:

```
final class UnitTestClass: Parsable {
  let friends: [UnitTestClass]
  let firstName: String?
  let lastName: String

  init(let friends: [UnitTestClass], let firstName: String?, let lastName: String, ) {
  ...
  init(dictionary: [String: AnyObject]) throws {
    ...
    self.firstName = try parser.fetchOptional("firstName")
  ...
  func data() -> [String: AnyObject] {
    ...
    data["friends"] = friends.map { $0.data() }
    data["firstName"] = firstName?.data()
    data["lastName"] = lastName.data()
    return data
  }
}
```

It is meant to be used with a parser who's current implementation is:
...
extension String: Parsable {
  static func parse(data: Any) throws -> String {
...
extension Int: Parsable {
  static func parse(data: Any) throws -> Int {
...
struct Parser {
  ...
  func fetch<T: Parsable>(key: String) throws -> T {
    ...
    return try T.parse(fetched)
  }

  func fetchOptional<T: Parsable>(key: String) throws -> T? {
    let fetchedOptional = dictionary?[key]
    guard let fetched = fetchedOptional else {
      return nil
    }
  ...
  func fetch<T: Parsable>(key: String) throws -> [T] {
    ...
    if let fetchedArray = fetched as? [AnyObject] {
      return try fetchedArray.map { try T.parse($0) }
    } else {
      throw ParserError(message: "The key '\(key)' was not found.")
    }
  }
