...
SwiftGen now supports `.xcstrings` files, allowing you to generate type-safe code for your Xcode Strings Catalog.

```yaml
strings:
  inputs: /path/to/Localizable.xcstrings
  outputs:
    templateName: structured-swift5
    output: Strings.swift
```

All `.strings` and `.stringsdict` files can be easily converted to `.xcstrings` files, which are used in Xcode for localization. This conversion allows developers to manage their localized strings more efficiently within the Xcode environment.

How to migrate? See [Discover String Catalogs WWDC Session](https://developer.apple.com/videos/play/wwdc2023/10155?time=1724)

<details>
<summary>Example of code generated by the structured bundled template</summary>

Given the following `Localizable.xcstrings` file:

| Key                  | Value | Comment |
| -------------------- |------------------------------- | -------------------------------- |
| alert_title          | Title of the alert             | Title for an alert 1️⃣            |
| bananas.owner        | Those %d bananas belong to %@. | A comment with no space above it |
| apples.count (Zero)  | You have no apples.            |                                  |
| apples.count (One)   | You have %d apple              |                                  |
| apples.count (Other) | You have %d apples             |                                  |
...
internal enum L10n {
  ...
  internal enum Apples {
    ...
    internal static func count(_ p1: Int) -> String {
      return L10n.tr("Localizable", "apples.count", p1, fallback: "Plural format key: apples.count")
    }
  }
...
or for `Localizable.strings` and `Localizable.stringsdict` files
