...
* [`static code` for anything](#static-code)
...
The model template also requires some static code which makes the generated code smaller. This static code is outputed by `swiftgen base -t model`. You should generate this code as well as the model code for each of your JSON specs.

If you do not like the model generated by this class, you can write your own spec and template. See [JSON spec](#json).

### Usage

<details>
<summary>Example input for the model template**</summary>

```json
{
  "type": "PersonModel",
  "fields": [
             {"type": "string", "optional": true, "name": "firstName", "doc": "First name of the person"},
             {"type": "string", "name": "lastName"},
             {"type": "array", "element": "MyModel", "name": "friends"},
             {"type": "array", "element": "MyModel", "name": "optFriends", "optional": true, "doc": "An optional array"},
             {"type": "int", "name": "age", "optional": true}
             ]
}
```
</details>

To `init` the model, you have a few options:
```swift
let dictionary: NSDictionary // this can be either NSDictionary or [String: Any]
// even if optional fields error because of missing required fields or type mismatch, this will still return a model
let person: PersonModel? = PersonModel(dictionary: dictionary)
// if optional fields error, this will still return a model but will also call the error block
let person: PersonModel? = PersonModel(dictionary: dictionary, error: { assertionFailure($0) })
// if any fields error in any way, this will simply throw
do {
  let person: PersonModel = PersonModel.make(dictionary: dictionary)
} catch {
  assertionFailure("Model parsing failed")
}
```

<details>
<summary>Example output for the model template**</summary>

```swift
// Generated using SwiftGen, by O.Halligon — https://github.com/AliSoftware/SwiftGen

public final class PersonModel: DataModel, Equatable {
  ...
  public let friends: [PersonModel]
  /**
   An optional array
   */
  public let optFriends: [PersonModel]?
  public let age: Int?

  // MARK: Initializers

  public init(firstName: String?, lastName: String, friends: [PersonModel], optFriends: [PersonModel]?, age: Int?) {
  ...
  public init?(dictionary: NSDictionary, error: ((String) throws -> Void)) rethrows {
    ...
    guard let lastName: String = try parser.fetch("lastName", error: error) else {
      try error("Required field 'lastName' is missing from type 'PersonModel'")
      return nil
    }
    self.lastName = lastName
  ...
  public static func make(withDictionary dictionary: NSDictionary) throws -> Self {
    let model = try self.init(dictionary: dictionary) { error in
      throw NSError(domain: "com.swiftgen.modelParsing", code: 0, userInfo: [ "parseMessage": error ])
    }
    if let model = model {
      return model
    } else {
      throw NSError(domain: "com.swiftgen.modelParsing", code: 0, userInfo: [ "parseMessage": "Model returned nil with no error." ])
    }
  }

  public static func parse(data: Any, error: (String) throws -> Void) rethrows -> PersonModel? {
  ...
  public func map(update: (Builder)->()) -> PersonModel {
  ...
  public final class Builder {
    ...
    public var friends: [PersonModel]
    public var optFriends: [PersonModel]?
    public var age: Int?

    public init(copy: PersonModel) {
    ...
    public func build() -> PersonModel {
      return PersonModel(firstName: firstName, lastName: lastName, friends: friends, optFriends: optFriends, age: age)
    }
  }
}

public func ==(lhs: , rhs: PersonModel) -> Bool {
...
SwiftGen doesn't just support model generation. You can use the `json` command to take any arbritrary JSON spec and template and generate anything. You can use this to generate different models, different swift code, or anything you can think of.

### Usage

```sh
swiftgen json /path/to/JSON/spec -t /path/to/template
```
...
SwiftGen has a command to generate static code with no input. This is useful for model generation as sometimes you want to output static helpers.

```sh
swiftgen base -t /path/to/static/template
```

The stencil context is always an empty dictionary for these templates.
