...
class FontsTests: XCTestCase {
  func testEmpty() {
    ...
    let template = GenumTemplate(templateString: Fixtures.string(for: "fonts-default.stencil"))
  ...
  func testDefaultsWithSwift3() {
    ...
    let template = GenumTemplate(templateString: Fixtures.string(for: "fonts-swift3.stencil"))
    let result = try! template.render(parser.stencilContext())
